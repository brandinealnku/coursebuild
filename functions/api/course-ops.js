...
const fail=(error,status=400,code="REQUEST_FAILED")=>json({ok:false,error:String(error||"Course Ops request failed."),code},status);
...
async function requestJson(url,init={}){
  ...
  if(!response.ok)throw new Error(body?.errors?.[0]?.message||body?.message||body?.error||`Canvas request failed (${response.status}).`);
...
async function verifyCourse(payload,env){
  const c=canvasConfig(payload,env);const {body}=await requestJson(courseUrl(c),{headers:canvasHeaders(c.token)});
  return{courseId:String(body.id||c.courseId),courseName:String(body.name||body.course_code||'').trim(),courseCode:String(body.course_code||''),canvasHost:c.host,courseUrl:body.html_url||`${c.base}/courses/${enc(c.courseId)}`};
}
function row(type,obj,moduleNames=[]){
  ...
  return{
    key:`${type}:${obj.id??obj.url??title}`,
    type,
    id:String(obj.id??obj.url??''),
    title,
    moduleNames,
    published:explicitlyUnpublished?false:published?true:null,
    dueAt:obj.due_at||null,
    unlockAt:obj.unlock_at||obj.delayed_post_at||null,
    lockAt:obj.lock_at||null,
    points:Number.isFinite(Number(obj.points_possible))?Number(obj.points_possible):null,
    updatedAt:obj.updated_at||obj.modified_at||null,
    htmlUrl:obj.html_url||obj.url||null
  };
}
async function inspectCourse(payload,env){
  ...
  const [modules,pages,assignments,discussions,files,quizzes,announcements]=await Promise.all([
    paged(courseUrl(c,'/modules?per_page=100'),c.token),
    paged(courseUrl(c,'/pages?per_page=100'),c.token),
    paged(courseUrl(c,'/assignments?per_page=100'),c.token),
    paged(courseUrl(c,'/discussion_topics?per_page=100'),c.token),
    paged(courseUrl(c,'/files?per_page=100'),c.token),
    paged(courseUrl(c,'/quizzes?per_page=100'),c.token),
    paged(announcementsUrl,c.token)
  ]);
  ...
  moduleItems.forEach(({module,items})=>items.forEach(i=>{
    const keys=[];if(i.content_id!=null)keys.push(`${String(i.type||'').toLowerCase()}:${i.content_id}`);if(i.page_url)keys.push(`page:${i.page_url}`);
    keys.forEach(k=>{const arr=moduleMap.get(k)||[];if(!arr.includes(module.name))arr.push(module.name);moduleMap.set(k,arr);});
  }));
  const modulesFor=(type,obj)=>{
    const keys=[`${type.toLowerCase()}:${obj.id}`];if(type==='Page')keys.push(`page:${obj.url}`);
    return [...new Set(keys.flatMap(k=>moduleMap.get(k)||[]))];
  };
  const rows=[
    ...modules.map(m=>row('Module',m,[])),
    ...pages.map(x=>row('Page',x,modulesFor('Page',x))),
    ...assignments.map(x=>row('Assignment',x,modulesFor('Assignment',x))),
    ...discussions.map(x=>row('Discussion',x,modulesFor('Discussion',x))),
    ...quizzes.map(x=>row('Quiz',x,modulesFor('Quiz',x))),
    ...files.map(x=>row('File',x,modulesFor('File',x))),
    ...announcements.map(x=>row('Announcement',x,[]))
  ];
  const counts=rows.reduce((out,r)=>(out[r.type]=(out[r.type]||0)+1,out),{});
  return{
    inspectedAt:new Date().toISOString(),
    course:{id:String(course.id||c.courseId),name:course.name||course.course_code||'',code:course.course_code||'',url:course.html_url||`${c.base}/courses/${enc(c.courseId)}`,host:c.host},
    counts,
    rows,
    readOnly:true,
    verification:'fresh Canvas read'
  };
}
...
export async function onRequestPost(context){
  ...
  }catch(error){return fail(error?.message||error,500,'COURSE_OPS_ERROR');}
}
export async function onRequestGet(context){
  const hosts=[...allowedCanvasHosts(context.env)];
  return ok({service:'Course Ops',status:'available',canvasConfigured:Boolean(context.env.CANVAS_ACCESS_TOKEN),approvedCanvasHosts:hosts,readOnly:true,version:'canvas-inspector-1'});
}
